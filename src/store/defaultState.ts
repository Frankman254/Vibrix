...
/**
 * Everything but the effect layers, which are derived from it below so the
 * starting layer cannot drift away from the default Looks values.
 */
...
export const DEFAULT_STATE: WallpaperState = {
	...
	motionLayers: [
		createDefaultMotionLayer(
			extractMotionLayerSettingsFromState(BASE_STATE),
			BASE_STATE.cameraMotionTargets
		)
	],
	activeEffectLayerId: DEFAULT_EFFECT_LAYER_ID,
	effectLayers: [
		createDefaultEffectLayer(
			extractFilterLookSettingsFromState(
				BASE_STATE as unknown as WallpaperState
			),
			BASE_STATE.filterTargets,
			BASE_STATE.activeFilterLookId
		)
	]
};
