...
| 0.5 | ✅ Decidido (store v116): el coverage clamp entra unconditional al resolve — se va el lock; `coverageFramingEdited` protege el encuadre manual del refit pasivo y los botones explícitos Cover Fit / Cover Fit All son la recalculación deliberada que sí lo sobrescribe | `resolveImageTransform`, `buildCoverFitPatch`, `autoFitCoveredActiveImage`              |
